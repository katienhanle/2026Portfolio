...
interface Props {
  ...
  /** When false, shows text statically (no ScrollTrigger auto-play). Default true. */
  playOnMount?: boolean;
}
  ...
  useGSAP(
    () => {
      ...
      const build = () => {
        ...
        chars.forEach((ch) => {
          ...
          const wrap = document.createElement("span") as unknown as HTMLElement;
      ...
      // ── playOnMount=false: show text statically, arm hover if needed ──
      if (!playOnMount) {
        if (triggerOnHover) {
          build();
          cleanupToStill();
      ...
      // ── playOnMount=true: ScrollTrigger-based auto-play ──
      const create = () => {
        build();
        if (scrambleCharset) randomizeScrambles();
        play();
    ...
    },
    {
      dependencies: [
        ...
        triggerOnHover, onShuffleComplete, playOnMount,
      ],
      scope: ref,
    }
  );
